...
export default function DoctorCheckinPage() {
  ...
  // Filter active cards
  const doctorCards = cards.filter(card => card.status === 'Active')
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl md:text-3xl font-bold text-[#006767]">Doctor Check-in</h1>
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto">
                {doctorCards.slice(0, 5).map((card) => (
                  <div key={card.id} className="flex items-center justify-between p-3 rounded-lg border hover:border-[#006767]/20 transition-colors">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {(card as any).appointment?.serviceName || "Consultation"}
                      </p>
                    </div>
                ...
                {doctorCards.length > 5 && (
                  <p className="text-center text-sm text-muted-foreground pt-2">
                    +{doctorCards.length - 5} more patients waiting
                  </p>
                )}
              </div>
            ) : (
