...
export default function DiagnosticCenterCheckinPage() {
  ...
  const diagnosticCards = (cards as CardWithAppointment[]).filter(card => card.status === 'Active')
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl md:text-3xl font-bold text-teal-600">Diagnostic Center Check-in</h1>
        <p className="text-muted-foreground">Verify and check in patients for diagnostic services</p>
      </div>

      <div className="grid gap-4 md:grid-cols-4">
        <StatsCard title="Total Active Cards" value={cardStats?.active || 0} icon={<Users className="h-5 w-5" />} variant="default" />
        <StatsCard title="Today's Check-ins" value={todayCheckIns.length} icon={<CheckCircle className="h-5 w-5" />} variant="success" />
        <StatsCard title="Total Used" value={cardStats?.used || 0} icon={<Clock className="h-5 w-5" />} variant="info" />
        <StatsCard title="Utilization Rate" value={`${cardStats?.utilizationRate || 0}%`} icon={<Calendar className="h-5 w-5" />} variant="primary" />
      </div>
          ...
          <CardContent>
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <Button className="bg-teal-600 hover:bg-teal-700" onClick={handleVerify} disabled={isVerifying || !cardNumber.trim() || checkInSuccess}>
              ...
              {(validationResult || foundCard || checkInResult) && (
                <div className={cn(
                  ...
                  checkInSuccess || checkInResult?.success ? "bg-green-50 border-green-200" 
                    : validationResult && !validationResult.isValid ? "bg-red-50 border-red-200"
                  ...
                  {checkInSuccess || checkInResult?.success ? (
                    <div className="space-y-2">
                      ...
                      <p className="text-green-600">Welcome, {checkInResult?.patientName || foundCard?.appointment?.patientName || "Patient"}</p>
                      <p className="text-sm text-green-600">Service: {checkInResult?.serviceName || foundCard?.appointment?.serviceName || "N/A"}</p>
                    </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">ID: {foundCard.appointment?.patientId || `APT-${foundCard.appointmentId}`}</p>
                          </div>
                        </div>
                      ...
                      <div className="grid gap-2 text-sm">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span>{foundCard.appointment?.scheduledDateTime ? new Date(foundCard.appointment.scheduledDateTime).toLocaleString() : "N/A"}</span>
                        </div>
                      </div>
                      <Button className="w-full bg-teal-600 hover:bg-teal-700" onClick={handleCheckIn}>
                    ...
                    <div className="space-y-2">
                      ...
                      <Button variant="outline" size="sm" onClick={() => { setCardNumber(""); clearValidationResult(); }}>Try Again</Button>
                    </div>
                  )}
                </div>
              )}
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto">
                {diagnosticCards.slice(0, 5).map((card) => (
                  <div key={card.id} className="flex items-center justify-between p-3 rounded-lg border hover:border-teal-200 transition-colors">
                    <div className="flex-1">
                      <p className="font-medium">{card.appointment?.patientName || `Appointment #${card.appointmentId}`}</p>
                      <p className="text-sm text-muted-foreground">{card.appointment?.serviceName || "Diagnostic Service"}</p>
                    </div>
                    <Badge className="bg-green-100 text-green-700">Ready</Badge>
                    <Button size="sm" variant="outline" className="ml-2" onClick={() => { setCardNumber(card.cardNumber); setTimeout(() => handleVerify(), 100); }}>Check In</Button>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-8 text-muted-foreground">No confirmed appointments for today</div>
            )}
          </CardContent>
        </Card>
      </div>
